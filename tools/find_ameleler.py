#!/usr/bin/env python3
"""find_ameleler — Kahya tool (subprocess). Keyword search over the index.

stdin:  {"q": "mail hatırlatma"}
stdout: JSON — eşleşen amelelerin kompakt listesi:
...
def main():
    ...
    try:
        ...
        rows = db.con.execute(
            "SELECT * FROM ameleler WHERE enabled = 1 ORDER BY name").fetchall()
        out = []
        for r in rows:
